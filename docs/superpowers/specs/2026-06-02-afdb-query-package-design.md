...
It also ships a **first-class batch runner** (concurrency + resumable on-disk caching +
a counts report) so the same operations run cleanly over many sequences in a data
pipeline.
